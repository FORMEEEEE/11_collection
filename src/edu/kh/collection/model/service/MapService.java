...
public class MapService { //살려주세요 시팔 정신나갈거같아
	...
	public void ex2() {
		...
		// VO(값 저장용 매체)는 특정 데이터 묶음의 재사용이 많은 경우 주로 사용
		...
		// Value == Object 타입 == 어떤 객체든 Value에 들어올 수 있음
		
		//값 세팅
		map.put("id", "user02");
		map.put("pw", "pass02");
		map.put("age", 25); // int -> Integer오토박싱
		
		// 값 출력
		System.out.println(map.get("id").toString() );
		// String.java.lang.object.toString() -> 정적 바인딩
		// 실행중 확인해보니 String 자식 객체 -> 자식 toString() 호출
		...
		// 향상된 for문
		for(String key :set) {
			System.out.println(map.get(key)); //map.get(key)매개변수 key는 돌아가면서 id,pw,age 요놈들을 한번씩 넣음 
												// ex) key ->id로 바뀌고 그뒤에 pw ...etc 
			//key가 반복될 때 마다 변경
